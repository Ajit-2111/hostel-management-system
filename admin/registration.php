...
if ($_POST['submit']) {
	...
	$query = "insert into  registration(roomno,Building,seater,feespm,fpm,frn,foodstatus,stayfrom,duration,course,semester,branch,regno,firstName,middleName,lastName,gender,contactno,emailid,password,egycontactno,guardianName,guardianRelation,guardianContactno,corresAddress,corresCIty,corresState,corresPincode,pmntAddress,pmntCity,pmnatetState,pmntPincode) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	$stmt = $mysqli->prepare($query);
	$rc = $stmt->bind_param('isiissisissssssssissississsisssi', $roomno, $bldname, $seater, $feespm, $fp, $frn, $foodstatus, $stayfrom, $duration, $course, $semester, $branch, $regno, $fname, $mname, $lname, $gender, $contactno, $emailid, $contactno, $emcntno, $gurname, $gurrelation, $gurcntno, $caddress, $ccity, $cstate, $cpincode, $paddress, $pcity, $pstate, $ppincode);
...
function checkAvailability() {
    ...
    jQuery.ajax({
        ...
        data: {
            roomno: $("#room").val(),
            Building: $("#bldname").val() // Add building parameter
        },
        type: "POST",
...
window.onerror = function(msg, url, linenumber) {
	...
	$.ajax({
				type: "POST",
				url: "get_seater.php",
				data: 'bldname=' + Building,
				success: function(data) {
					console.log(data);
					
						 $("#bldname option").not(":first").remove();
					$('#bldname').append(data);
				}
			});
}

	function getSeater2(val) {
	    	   Building=val;  
	$.ajax({
    type: "POST",
    url: "get_seater.php",
...
<body>
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													<select name="room" id="room" class="form-control" onChange="getSeater(this.value);"  required>
														...
														<?php $query = "SELECT DISTINCT room_no FROM rooms ORDER BY room_no ASC";
    ...
    <div class="col-sm-8">
        <select name="bldname" id="bldname" class="form-control" onChange="getSeater2(this.value);" onBlur="checkAvailability()" required>
                <option value="">Select Building</option>
            <div id="bldnameopt"></div>
        </select>
        <span id="room-availability-status" style="font-size:12px;"></span>
											...
											<div class="form-group">
												<label class="col-sm-2 control-label">Fee Per Year</label>
												<div class="col-sm-8">
													<input type="number" name="fpm" id="fpm" class="form-control" value="24000">
												</div>
											</div>

											<div class="form-group">
												<label class="col-sm-2 control-label">fee paid</label>
												<div class="col-sm-8">
													<input type="number" name="fp" id="fp" value="0" class="form-control">
												</div>
											</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													<input type="text" name="frn" id="frn" class="form-control">
												</div>
											</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													<input type="tel" name="econtact" id="econtact" class="form-control" required="required">
												</div>
											</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													<input type="text" name="grelation" id="grelation" class="form-control" value="">
												</div>
											</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													<input type="tel" name="gcontact" id="gcontact" class="form-control" value="">
												</div>
											</div>
